...
from app.types.enumerations.users import GetUserFilter
from app.types.models.cards import Card, InternalCard
from app.types.models.users import User, InputUser
...
class UsersController(CardsController):
    ...
    def get_users(self) -> tuple[User, ...]:
        ...
        with super().connection.cursor(buffered=True) as cursor:
            ...
            all_dirty_users_ids = cursor.fetchall()
            all_users: list[User, ...] = [
                self.get_user(payload=int(user_id[0])) for user_id in all_dirty_users_ids
            ]
    ...
    def is_user(self, payload: Any, filter_: GetUserFilter = GetUserFilter.by_user_id) -> bool:
        ...
        with super().connection.cursor(buffered=True) as cursor:
            cursor.execute(filters_and_queries[filter_], (payload,))
            matches_found = cursor.fetchone()[0]
            return bool(matches_found)
